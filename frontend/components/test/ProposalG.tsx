"use client";
/* ProposalG — 골드 나이트
   딥 네이비 배경 · 골드/앰버 네온 · 풍부한 글래스모피즘 · 고급스러운 분위기 */
...
const GN  = "2px -0.2px 5px rgba(251,191,36,0.18), 3px -0.3px 9px rgba(251,191,36,0.09)";
const GNA = "2px -0.2px 5px rgba(251,191,36,0.45), 3px -0.3px 9px rgba(251,191,36,0.22), 0 0 10px rgba(251,191,36,0.15)";
...
function Tile({ name, icon, active, onClick }:{name:string;icon:string;active:boolean;onClick:()=>void}) {
  return (
    <button onClick={onClick} style={{boxShadow: active ? GNA : GN}}
      className={`shrink-0 w-[52px] h-[52px] rounded-xl flex flex-col items-center justify-between py-1.5 px-1 transition-all duration-200 ${
        active ? "bg-amber-500/55 text-white" : "bg-white/8 backdrop-blur-md text-white/60"
      }`}>
...
export default function ProposalG() {
  ...
  return (
    <div className="relative h-full overflow-hidden"
      style={{background:"linear-gradient(145deg,#080c1c 0%,#0e1635 50%,#0a1020 100%)"}}>

      {/* 별 파티클 효과 */}
      <div className="absolute inset-0 opacity-30"
        style={{backgroundImage:"radial-gradient(circle,rgba(251,191,36,0.6) 1px,transparent 1px)",backgroundSize:"80px 80px"}}/>

      <div className="absolute inset-0 overflow-y-auto [&::-webkit-scrollbar]:hidden"
           style={{paddingTop:`${MENU_H+16}px`,paddingBottom:"80px"}}>
        {layout==="list" ? <GList /> : <GBox />}
      </div>

      {/* 상단 메뉴 */}
      <div className="absolute top-0 left-0 right-0 z-20"
        style={{height:`${MENU_H}px`,
          background:"linear-gradient(to bottom,rgba(8,12,28,0.96) 0%,rgba(8,12,28,0.85) 80%,transparent 100%)",
          backdropFilter:"blur(22px)",WebkitBackdropFilter:"blur(22px)"}}>

        <div className="flex items-center gap-2 px-4 pt-2 pb-1">
          <span className="text-base">👑</span>
          <span className="font-black text-sm text-amber-400 tracking-tight">Shorts100</span>
          <span className="text-[10px] font-semibold px-2 py-0.5 rounded-full bg-amber-500/15 text-amber-400">TOP 100</span>
        </div>

        <div className="px-4 mb-1.5">
          <p className="text-[9px] font-bold mb-1 tracking-widest uppercase text-amber-900">나라</p>
          <div className={SCROLL}>
            {REGION_DATA.map(([n,i])=><Tile key={n} name={n} icon={i} active={region===n} onClick={()=>setRegion(n)}/>)}
          </div>
        </div>
        <div className="px-4 mb-1.5">
          <p className="text-[9px] font-bold mb-1 tracking-widest uppercase text-amber-900">카테고리</p>
          <div className={SCROLL}>
            {CAT_DATA.map(([n,i])=><Tile key={n} name={n} icon={i} active={cat===n} onClick={()=>setCat(n)}/>)}
          </div>
        </div>
        <div className="px-4">
          <p className="text-[9px] font-bold mb-1 tracking-widest uppercase text-amber-900">기간</p>
      ...
      <div className="absolute bottom-2 left-0 right-0 z-30 flex justify-center">
        <div className="w-28 h-1 rounded-full bg-amber-900/40" />
      </div>
    </div>
  );
}
...
function GList() {
  return (
    <div className="px-3 space-y-2 pb-2">
      {DUMMY.map((item)=>(
        <div key={item.rank}
          style={{boxShadow:"2px -0.2px 5px rgba(251,191,36,0.12),3px -0.3px 9px rgba(251,191,36,0.06)"}}
          className="flex items-center gap-3 bg-white/6 backdrop-blur-sm rounded-2xl px-3 py-2.5 border border-white/6">
          <span className={`w-7 h-7 rounded-full flex items-center justify-center text-xs font-black shrink-0 ${
            item.rank===1?"bg-yellow-400 text-gray-900":item.rank===2?"bg-gray-500 text-white":item.rank===3?"bg-amber-500 text-white":"bg-amber-900/40 text-amber-400"
          }`}>{item.rank}</span>
          ...
          <div className="flex-1 min-w-0">
            ...
            <div className="flex gap-1.5 mt-0.5 text-[10px] text-white/40">
              ...
              <span className="bg-amber-900/40 text-amber-400 px-1 rounded-md text-[9px]">{item.region}</span>
            </div>
          </div>
...
function GBox() {
  return (
    <div className="grid grid-cols-2 gap-2 px-2">
      {DUMMY.map((item)=>(
        <div key={item.rank}
          style={{boxShadow:"2px -0.2px 5px rgba(251,191,36,0.16),3px -0.3px 9px rgba(251,191,36,0.08)"}}
          ...
          <div className={`absolute top-2 left-2 w-7 h-7 rounded-full flex items-center justify-center text-xs font-black z-10 ${
            item.rank===1?"bg-yellow-400 text-gray-900 scale-125":item.rank<=3?"bg-amber-500 text-white":"bg-black/60 text-white"
          }`}>{item.rank}</div>
          <div className="absolute bottom-0 left-0 right-0 bg-gradient-to-t from-black/85 via-black/50 to-transparent px-2 pt-8 pb-2.5 z-10">
            <p className="text-amber-400/60 text-[9px]">@{item.channel}</p>
